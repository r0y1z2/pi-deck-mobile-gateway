...
export function getDeckDevicesPath(): string {
	return join(getServerDir(), "deck-devices.json");
}

export function getSocketPath(): string {
	if (process.platform === "win32") {
		const pipeName = getServerDir().replace(/[^a-zA-Z0-9_.-]/g, "_");
		return `\\\\.\\pipe\\${pipeName}-server.sock`;
	}
