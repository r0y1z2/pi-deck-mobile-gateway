...
export function saveInstances(instances: InstanceRecord[]): void {
	ensureServerDir();
	const instancesPath = getInstancesPath();
	const temporaryPath = `${instancesPath}.${process.pid}.tmp`;
	writeFileSync(temporaryPath, JSON.stringify(instances, null, 2), { mode: 0o600 });
	renameSync(temporaryPath, instancesPath);
}
